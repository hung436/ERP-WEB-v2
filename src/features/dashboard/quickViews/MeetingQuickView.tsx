...
export function MeetingQuickView({ event, onClose }: { event: MeetingEvent; onClose: () => void }) {
  ...
  return (
    <Modal
      centered
      className="online-meeting-modal"
      footer={
        <div className="document-modal-actions">
          <Button onClick={onClose}>Đóng</Button>
          <Button icon={<ModuleIcon module="meetings" size={16} />} onClick={joinMeeting} type="primary">
    ...
    >
      <article className="meeting-modal-content">
        <header className="meeting-modal-summary">
          <p>📹 Nền tảng: <strong>{event.platform}</strong> · ID phòng: <strong>{event.meetingId ?? 'TT-ONLINE'}</strong></p>
        </header>
        <section className="meeting-time-panel">
          <div>
            <small>Bắt đầu</small>
        ...
        <dl>
          <div>
            <dt>Người chủ trì / Tổ chức</dt>
            <dd>{event.organizer}</dd>
          </div>
          <div>
            <dt>Nền tảng trực tuyến</dt>
            <dd>{event.platform}</dd>
          </div>
          <div>
            <dt>Đường dẫn phòng họp</dt>
            <dd>{event.meetingUrl ?? 'https://meeting.tuoitre.vn'}</dd>
          </div>
          <div>
            <dt>Ghi hình cuộc họp</dt>
            <dd>{event.recordingAvailable ? 'Có ghi hình tự động' : 'Không ghi hình'}</dd>
          </div>
        </dl>
        <section className="meeting-agenda">
          <h3>Chương trình / Nội dung họp trực tuyến</h3>
          <p>{event.agenda ?? 'Chưa cập nhật nội dung.'}</p>
        </section>
        <section className="meeting-participants">
          <h3>Thành phần tham dự <span>{event.participants?.length ?? 0}</span></h3>
          <div>
