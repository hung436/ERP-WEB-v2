...
export function ChatQuickView({ chat, onClose }: { chat: ChatConversation; onClose: () => void }) {
  ...
  return <Modal centered className="dashboard-preview workspace-preview chat-preview-modal" footer={null} onCancel={onClose} open title={<span className="chat-quick-title"><span className={`chat-preview-avatar ${avatarTone(chat.participantName)}`}>{chat.participantName.slice(0, 2).toUpperCase()}<i className={chat.online ? 'online' : ''} /></span><span><strong>{chat.participantName}</strong><small className={chat.online ? 'online' : ''}><i />{presence}</small></span><small className="quick-badge-tag">Nhắn tin nhanh</small>{chat.unreadCount > 0 && <b>{chat.unreadCount}</b>}</span>} width={700}>
