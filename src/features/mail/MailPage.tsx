...
const folders: { key: Folder; label: string; icon: string }[] = [
  ...
  { key: 'sent', label: 'Đã gửi', icon: '➤' }, { key: 'drafts', label: 'Bản nháp', icon: '▤' },
  { key: 'archive', label: 'Lưu trữ', icon: '□' }, { key: 'spam', label: 'Thư rác', icon: '!' },
  { key: 'trash', label: 'Thùng rác', icon: '⌫' },
];
...
export function MailPage() {
  ...
  const attachFiles = (files: FileList | null) => setAttachments(files ? Array.from(files).map((file, index) => ({ id: `mail-file-${Date.now()}-${index}`, name: file.name, size: file.size, type: file.type || 'application/octet-stream' })) : []);
    ...
    <section className="gmail-mail-shell surface-panel">
      ...
      <article className="gmail-detail">
        {detail.loading ? <ContentSkeleton rows={8} /> : detail.error ? <ErrorState message={detail.error} /> : detail.data?.folder === 'drafts' ? <section className="gmail-draft-preview">
          ...
          <p>{detail.data.body || 'Bản nháp này chưa có nội dung.'}</p>
        ...
        </section> : detail.data ? <><div className="gmail-detail-toolbar"><Tooltip title="Lưu trữ"><Button aria-label="Lưu trữ Mail" icon={<MailActionIcon name="archive" />} loading={acting === 'archive'} onClick={() => void runAction('archive')} /></Tooltip><Tooltip title={detail.data.isRead ? 'Đánh dấu chưa đọc' : 'Đánh dấu đã đọc'}><Button aria-label={detail.data.isRead ? 'Đánh dấu chưa đọc' : 'Đánh dấu đã đọc'} icon={<MailActionIcon name="unread" />} onClick={() => void runAction(detail.data?.isRead ? 'mark-unread' : 'mark-read')} /></Tooltip><Tooltip title="Báo cáo thư rác"><Button aria-label="Báo cáo thư rác" icon={<MailActionIcon name="spam" />} onClick={() => void runAction('spam')} /></Tooltip><Tooltip title="Xóa"><Button danger aria-label="Xóa Mail" icon={<MailActionIcon name="trash" />} onClick={() => void runAction('trash')} /></Tooltip></div><header><span className="eyebrow">{detail.data.folder === 'sent' ? 'Thư đã gửi' : 'Thư đến'}</span><h2>{detail.data.subject}</h2><div><span className="avatar neutral">{detail.data.senderName.slice(0, 2).toUpperCase()}</span><span><strong>{detail.data.senderName}</strong><small>{detail.data.senderEmail} · tới {detail.data.recipients?.join(', ') || 'tôi'}</small></span><time>{new Date(detail.data.sentAt).toLocaleString('vi-VN')}</time></div></header><div className="mail-body">{detail.data.body}</div>{Boolean(detail.data.attachments?.length) && <section className="gmail-attachments">{detail.data.attachments?.map((file) => <button key={file.id} type="button"><MailFileIcon file={file} /><span><strong>{file.name}</strong><small>{(file.size / 1024).toFixed(0)} KB</small></span></button>)}</section>}<footer className="gmail-reply-actions"><Button icon={<MailActionIcon name="reply" />} onClick={() => openComposer('reply')}>Trả lời</Button><Button icon={<MailActionIcon name="replyAll" />} onClick={() => openComposer('reply-all')}>Trả lời tất cả</Button><Button icon={<MailActionIcon name="forward" />} onClick={() => openComposer('forward')}>Chuyển tiếp</Button></footer></> : <EmptyState description="Chọn một Mail để đọc" />}
      </article>
    </section>
    <Modal centered className={`gmail-compose-modal ${composer ?? 'compose'}-mode`} footer={<div className="mail-compose-footer"><Button onClick={() => void saveDraft()}>Lưu bản nháp</Button><span className="draft-auto-save">Đóng cửa sổ sẽ tự lưu bản nháp</span><Button onClick={() => void closeComposer()}>Đóng</Button><Button loading={sending} onClick={() => form.submit()} type="primary">Gửi</Button></div>} onCancel={() => void closeComposer()} open={Boolean(composer)} title={<span className="mail-compose-heading"><span className="section-icon mail"><ModuleIcon module="mail" size={19} /></span><span><strong>{composerTitle}</strong><small>{composerHint}</small></span></span>} width={720}>
      <Form className="mail-compose-form" form={form} layout="vertical" onFinish={(values) => void send(values)}>
        <Form.Item label="Người nhận" name="recipients" rules={[{ required: true, message: 'Nhập ít nhất một người nhận' }]}><Input addonAfter={<button className="cc-toggle" onClick={() => setShowCarbonCopy((value) => !value)} type="button">Cc/Bcc</button>} placeholder="Email người nhận, phân cách bằng dấu phẩy" /></Form.Item>
        {showCarbonCopy && <div className="gmail-cc-grid"><Form.Item label="Cc" name="cc"><Input /></Form.Item><Form.Item label="Bcc" name="bcc"><Input /></Form.Item></div>}
        <Form.Item label="Chủ đề" name="subject" rules={[{ required: true, message: 'Nhập chủ đề' }]}><Input /></Form.Item>
        <Form.Item label="Nội dung" name="content" rules={[{ required: true, message: 'Nhập nội dung' }]}><Input.TextArea autoSize={{ minRows: 3, maxRows: 6 }} rows={4} /></Form.Item>
        <label className="gmail-file-dropzone">
          <span className="dropzone-icon"><MailActionIcon name="attachment" /></span>
          <div className="dropzone-copy">
            <strong>Đính kèm tệp văn bản hoặc hình ảnh</strong>
            <small>Kéo thả tệp hoặc nhấp vào đây để chọn tệp (PDF, DOCX, XLSX, hình ảnh, tệp nén · Tối đa 10 MB)</small>
          </div>
          <input multiple onChange={(event) => attachFiles(event.target.files)} type="file" />
        </label>
        {Boolean(attachments.length) && <div className="gmail-compose-files">{attachments.map((file) => <span key={file.id}><MailFileIcon file={file} /><span><strong>{file.name}</strong><small>{(file.size / 1024).toFixed(0)} KB · {file.type || 'Tệp dữ liệu'}</small></span><button aria-label={`Bỏ tệp ${file.name}`} onClick={() => setAttachments((current) => current.filter((item) => item.id !== file.id))} type="button">×</button></span>)}</div>}
      </Form>
    </Modal>
  </div>;
}
